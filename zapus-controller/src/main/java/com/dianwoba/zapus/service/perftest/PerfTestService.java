...
import static com.dianwoba.zapus.common.constants.MonitorConstants.MONITOR_FILE_PREFIX;
import static com.dianwoba.zapus.common.util.AccessUtils.getSafe;
import static com.dianwoba.zapus.common.util.CollectionUtils.newArrayList;
import static com.dianwoba.zapus.common.util.CollectionUtils.newHashMap;
import static com.dianwoba.zapus.common.util.CollectionUtils.newHashSet;
...
import static com.dianwoba.zapus.model.Status.getProcessingOrTestingTestStatus;

import com.dianwoba.zapus.common.constant.ControllerConstants;
import com.dianwoba.zapus.common.constants.GrinderConstants;
import com.dianwoba.zapus.common.util.ObjectUtils;
import com.dianwoba.zapus.handler.NullScriptHandler;
...
import com.dianwoba.zapus.mapper.ZapusPerftestMapperExt;
import com.dianwoba.zapus.mapper.ZapusUserMapper;
import com.dianwoba.zapus.mapper.ZapusUserMapperExt;
import com.dianwoba.zapus.model.Permission;
import com.dianwoba.zapus.model.RampUp;
...
import com.dianwoba.zapus.model.perftest.PerfTestStatistics;
import com.dianwoba.zapus.model.perftest.ProcessAndThread;
import com.dianwoba.zapus.model.script.FileEntry;
import com.dianwoba.zapus.monitor.controller.model.SystemDataModel;
import com.dianwoba.zapus.perftest.repository.PerfTestRepository;
import com.dianwoba.zapus.service.AbstractPerfTestService;
...
public class PerfTestService extends AbstractPerfTestService implements ControllerConstants,
    GrinderConstants {
    ...
    @Resource
    private ZapusPerftestMapperExt zapusPerftestMapperExt;

    @Resource
    private ZapusUserMapperExt zapusUserMapperExt;
     ...
     * @param query
     * @param tag
     * @param queryFilter S/F/R
     * @param pageable
     * @return
     */
    public List<ZapusPerftest> getPagedAll(ZapusUser user, String query, String tag, String queryFilter,
            Pageable pageable) {
        ZapusPerftestExample example = new ZapusPerftestExample();
        ZapusPerftestExample.Criteria criteria = example.createCriteria();

        if (user.getRoleName().equals(Role.USER)) {
            criteria.andCreateUserIdEqualTo(user.getId());
        }

        if (StringUtils.isNotBlank(tag)) {
            criteria.andTagStringEqualTo(tag);
        }
        if ("F".equals(queryFilter)) {
            criteria.andStatusEqualTo(Status.FINISHED.name());
        } else if ("R".equals(queryFilter)) {
            criteria.andStatusEqualTo(Status.TESTING.name());
        } else if ("S".equals(queryFilter)) {
            criteria.andStatusEqualTo(Status.READY.name());
            criteria.andScheduledTimeIsNotNull();
        }
        if (StringUtils.isNotBlank(query)) {
            criteria.andTestNameLike(query);
        }
        return zapusPerftestMapperExt.selectByExampleWithPaging(example, pageable.getOffset(),
            pageable.getPageSize());
    }

    /**
     * Get {@link ZapusPerftest} list on the user.
     *
     * @param user user
     * @return found {@link ZapusPerftest} list
     */
    List<ZapusPerftest> getAll(ZapusUser user) {
        ZapusPerftestExample example = new ZapusPerftestExample();
        ZapusPerftestExample.Criteria criteria = example.createCriteria();
        criteria.andCreateUserIdEqualTo(user.getId());
        return zapusPerftestMapperExt.selectByExample(example);
    }


    @Override
    public ZapusPerftest getOne(User user, Long id) {
        ZapusPerftestExample example = new ZapusPerftestExample();
        ZapusPerftestExample.Criteria criteria = example.createCriteria();
        ...
        List<ZapusPerftest> zapusPerftests = zapusPerftestMapperExt.selectByExample(example);
        if (CollectionUtils.isNotEmpty(zapusPerftests)) {
            return zapusPerftests.get(0);
        }
        return null;
    }

    @Override
    public List<ZapusPerftest> getAll(User user, Long[] ids) {
        if (ids.length == 0) {
            return newArrayList();
        }
        ZapusPerftestExample example = new ZapusPerftestExample();
    ...
    @Override
    public long count(User user, List<String> statuses) {
        if (statuses.isEmpty()) {
            return 0;
        } else {
            ZapusPerftestExample example = new ZapusPerftestExample();
            ...
            return zapusPerftestMapperExt.countByExample(example);
        }

    }

    @Override
    public List<ZapusPerftest> getAll(User user, List<String> statuses) {
        ZapusPerftestExample example = new ZapusPerftestExample();
        ZapusPerftestExample.Criteria criteria = example.createCriteria();
        if (user != null) {
            criteria.andCreateUserIdEqualTo(user.getId());
    ...
    private List<ZapusPerftest> getAll(User user, String region, List<String> statuses) {
        ZapusPerftestExample example = new ZapusPerftestExample();
        ZapusPerftestExample.Criteria criteria = example.createCriteria();
        if (user != null) {
            criteria.andCreateUserIdEqualTo(user.getId());
    ...
    @Override
    @Transactional
    public ZapusPerftest save(User user, ZapusPerftest perfTest) {
    ...
    private ZapusPerftest save(ZapusPerftest perfTest) {
        checkNotNull(perfTest);
        // Merge if necessary
        if (perfTest.getId() != null && perfTest.getId() != 0) {
            ZapusPerftest existingPerfTest = zapusPerftestMapperExt.selectByPrimaryKey(perfTest.getId());
    ...
    private void attachFileRevision(User user, ZapusPerftest perfTest) {
    ...
    private void attachTags(User user, ZapusPerftest perfTest, String tagString) {
    ...
    /**
     * Update runtime statistics on {@link ZapusPerftest} having the given id.
     *
     * @param id id of {@link ZapusPerftest}
    ...
    /**
     * Mark test error on {@link ZapusPerftest} instance.
     *
     * @param perfTest {@link ZapusPerftest}
     * @param reason stop reason
     * @return perftest with updated data
     */
    @Transactional
    public ZapusPerftest markAbnormalTermination(ZapusPerftest perfTest, StopReason reason) {
     ...
     */
    @Transactional
    public ZapusPerftest markAbnormalTermination(ZapusPerftest perfTest, String reason) {
    ...
    @Transactional
    @Override
    public ZapusPerftest markStatusAndProgress(ZapusPerftest perfTest, Status status, String message) {
     ...
     */
    @Transactional
    public ZapusPerftest markProgress(ZapusPerftest perfTest, String message) {
        checkNotNull(perfTest);
     ...
     */
    @Transactional
    public ZapusPerftest markProgressAndStatus(ZapusPerftest perfTest, Status status, String message) {
     ...
     */
    @Transactional
    public ZapusPerftest markProgressAndStatusAndFinishTimeAndStatistics(ZapusPerftest perfTest,
     ...
     */
    @Transactional
    public ZapusPerftest markPerfTestConsoleStart(ZapusPerftest perfTest, int consolePort) {
        perfTest.setPort(consolePort);
        return markProgressAndStatus(perfTest, Status.START_CONSOLE_FINISHED,
            "Console is started on port "
                + consolePort);
    }
    ...
    @Transactional
    @Override
    public ZapusPerftest getOneWithTag(Long testId) {
        ZapusPerftest findOne = perfTestRepository.findOne(testId);
        if (findOne != null) {
    ...
    @Override
    public ZapusPerftest getOne(Long testId) {
        return perfTestRepository.findOne(testId);
    }

    /**
     * Get the next runnable {@link ZapusPerftest}.
     *
     * @return found {@link ZapusPerftest} which is ready to run, null otherwise
     */
    @Transactional
    public ZapusPerftest getNextRunnablePerfTestPerfTestCandidate() {
        List<ZapusPerftest> readyPerfTests = perfTestRepository
            .findAllByStatusOrderByScheduledTimeAsc(Status.READY);
        List<ZapusPerftest> usersFirstPerfTests = filterCurrentlyRunningTestUsersTest(readyPerfTests);
        return usersFirstPerfTests.isEmpty() ? null : readyPerfTests.get(0);
    }

    /**
     * Get currently running {@link ZapusPerftest} list.
     *
     * @return running test list
     */
    public List<ZapusPerftest> getCurrentlyRunningTest() {
        return getAll(null, Status.getProcessingOrTestingTestStatus());
    }

    /**
     * Filter out {@link ZapusPerftest} whose owner is running another test now..
     *
     * @param perfTestLists perf test
     * @return filtered perf test
     */
    protected List<ZapusPerftest> filterCurrentlyRunningTestUsersTest(List<ZapusPerftest> perfTestLists) {
        List<ZapusPerftest> currentlyRunningTests = getCurrentlyRunningTest();
        final Set<User> currentlyRunningTestOwners = newHashSet();
        for (ZapusPerftest each : currentlyRunningTests) {
            currentlyRunningTestOwners.add(each.getCreatedUser());
        }
        CollectionUtils.filter(perfTestLists, new Predicate() {
            @Override
            public boolean evaluate(Object object) {
                ZapusPerftest perfTest = (ZapusPerftest) object;
    ...
    @Override
    public List<ZapusPerftest> getAllTesting() {
     ...
     * 获取反常测试
     */
    public List<ZapusPerftest> getAllAbnormalTesting() {
        return getAll(null, config.getRegion(), new Status[]{Status.ABNORMAL_TESTING});
    }

    /**
     * Delete test {@link ZapusPerftest} by user and test id.
     *
     * @param user user
     * @param id test id
     */
    ...
    public void delete(User user, long id) {
        ZapusPerftest perfTest = getOne(id);
    ...
    /**
     * Delete {@link ZapusPerftest} directory.
     *
     * @param perfTest perfTest
     */
    private void deletePerfTestDirectory(ZapusPerftest perfTest) {
     ...
     * (non-Javadoc)
     *
     * @see com.dianwoba.zapus.service.IPerfTestService#getPerfTestFilePath(org .ngrinder.perftest. model.ZapusPerftest)
     */
    @Override
    public File getStatisticPath(ZapusPerftest perfTest) {
     ...
     */
    @Override
    public File getDistributionPath(ZapusPerftest perfTest) {
        return config.getHome().getPerfTestDistDirectory(perfTest);
    }

    /**
     * Build custom class path on the given {@link ZapusPerftest}.
     *
     * @param perfTest perftest
    ...
    public String getCustomClassPath(ZapusPerftest perfTest) {
    ...
    /**
     * Create {@link GrinderProperties} based on the passed {@link ZapusPerftest}.
     *
     * @param perfTest base data
     * @return created {@link GrinderProperties} instance
     */
    public GrinderProperties getGrinderProperties(ZapusPerftest perfTest) {
     ...
     * @return created {@link GrinderProperties} instance
     */
    public GrinderProperties getGrinderProperties(ZapusPerftest perfTest, ScriptHandler scriptHandler) {
        try {
            ...
            grinderProperties
                .setProperty(GRINDER_PROP_SCRIPT, scriptHandler.getScriptExecutePath(scriptName));
            ...
            if (BooleanUtils.isTrue(perfTest.getUseRampUp())) {
                grinderProperties.setBoolean(GRINDER_PROP_THREAD_RAMPUP,
                    perfTest.getRampUpType() == RampUp.THREAD);
                grinderProperties
                    .setInt(GRINDER_PROP_PROCESS_INCREMENT, getSafe(perfTest.getRampUpStep()));
                ...
                grinderProperties
                    .setInt(GRINDER_PROP_INITIAL_PROCESS, getSafe(perfTest.getRampUpInitCount()));
            } else {
            ...
            grinderProperties
                .setInt(GRINDER_PROP_IGNORE_SAMPLE_COUNT, getSafe(perfTest.getIgnoreSampleCount()));
            ...
            if (StringUtils.isNotBlank(perfTest.getParam())) {
                ...
                if (config.isSecurityEnabled()) {
                    grinderProperties
                        .setProperty(GRINDER_PROP_PARAM, StringUtils.trimToEmpty(param));
                } else {
            ...
            throw processException(
                "error while prepare grinder property for " + perfTest.getTestName(), e);
        }
    }

    /**
     * Prepare files for distribution. This method stores the files on the path
     * ${ZAPUS_HOME}/perftest/{test_id}/dist folder.
     *
     * @param perfTest perfTest
     * @return File location in which the perftest script and resources are distributed.
     */
    public ScriptHandler prepareDistribution(ZapusPerftest perfTest) {
        ...
        handler.prepareDist(perfTest.getId(), user, scriptEntry, perfTestDistDirectory,
            config.getControllerProperties(),
            processingResult);
    ...
    public ProcessAndThread calcProcessAndThread(int newVuser) {
        try {
            ...
            engine.eval(script);
            int processCount = ((Double) engine.eval("getProcessCount(" + newVuser + ")"))
                .intValue();
     ...
     */
    public File getCsvReportFile(ZapusPerftest perfTest) {
    ...
    public File getLogFileDirectory(ZapusPerftest perfTest) {
     ...
     */
    public File getReportFileDirectory(ZapusPerftest perfTest) {
    ...
    String getProperSizedStatusString(Map<String, SystemDataModel> agentStatusMap) {
        ...
        if (statusLength > 9950) { // max column size is 10,000
            LOGGER
                .info("Agent status string length: {}, too long to save into table.", statusLength);
    ...
    public Map<String, Object> getStatistics(ZapusPerftest perfTest) {
    ...
    public Map<String, HashMap> getAgentStat(ZapusPerftest perfTest) {
     ...
     * (non-Javadoc)
     *
     * @see com.dianwoba.zapus.service.IPerfTestService#getAllPerfTest()
     */
    @Override
    public List<ZapusPerftest> getAllPerfTest() {
        return perfTestRepository.findAll();
    }

    /**
     * Create {@link ConsoleProperties} based on given {@link ZapusPerftest} instance.
     *
     * @param perfTest perfTest
     * @return {@link ConsoleProperties}
     */
    public ConsoleProperties createConsoleProperties(ZapusPerftest perfTest) {
        ConsoleProperties consoleProperties = ConsolePropertiesFactory
            .createEmptyConsoleProperties();
        try {
            consoleProperties
                .setAndSaveDistributionDirectory(new Directory(getDistributionPath(perfTest)));
     ...
     */
    @SuppressWarnings("unchecked")
    public boolean hasTooManyError(ZapusPerftest perfTest) {
        Map<String, Object> result = getStatistics(perfTest);
        Map<String, Object> totalStatistics = MapUtils
            .getMap(result, "totalStatistics", MapUtils.EMPTY_MAP);
    ...
    /**
     * Update the given {@link ZapusPerftest} properties after test finished.
     *
     * @param perfTest perfTest
     */
    public void updatePerfTestAfterTestFinish(ZapusPerftest perfTest) {
        checkNotNull(perfTest);
        Map<String, Object> result = consoleManager.getConsoleUsingPort(perfTest.getPort())
            .getStatisticsData();
        @SuppressWarnings("unchecked")
        Map<String, Object> totalStatistics = MapUtils
            .getMap(result, "totalStatistics", MapUtils.EMPTY_MAP);
    ...
    public boolean canExecuteTestMore() {
        return count(null, Status.getProcessingOrTestingTestStatus())
            < getMaximumConcurrentTestCount();
    }

    /*
     * (non-Javadoc)
     *
     * @see com.dianwoba.zapus.service.IPerfTestService#stop(com.dianwoba.zapus .model.User, java.lang.Long)
     */
    @Override
    @Transactional
     ...
     */
    public boolean hasPermission(ZapusPerftest perfTest, User user, Permission type) {
        return perfTest != null && (user.getRole().hasPermission(type) || user
            .equals(perfTest.getCreatedUser()));
    }

    /*
     * (non-Javadoc)
     *
     * @see com.dianwoba.zapus.service.IPerfTestService#getAllStopRequested()
     */
    @Override
    public List<ZapusPerftest> getAllStopRequested() {
        final List<ZapusPerftest> perfTests = getAll(null, config.getRegion(),
            getProcessingOrTestingTestStatus());
        CollectionUtils.filter(perfTests, new Predicate() {
            @Override
            public boolean evaluate(Object object) {
                return (((ZapusPerftest) object).getStopRequest() == Boolean.TRUE);
            }
        });
        return perfTests;
    }
     ...
     * (non-Javadoc)
     *
     * @see com.dianwoba.zapus.service.IPerfTestService#addCommentOn(com.dianwoba.zapus .model.User, int, java.lang.String)
     */
    @Override
    @Transactional
    public void addCommentOn(User user, Long testId, String testComment, String tagString) {
        ZapusPerftest perfTest = getOne(user, testId);
    ...
    public Collection<PerfTestStatistics> getCurrentPerfTestStatistics() {
        Map<ZapusUser, PerfTestStatistics> perfTestPerUser = newHashMap();
        for (ZapusPerftest each : getAll(null, getProcessingOrTestingTestStatus())) {
            ZapusUser lastModifiedUser = zapusUserMapperExt.selectByPrimaryKey(each.getCreateUserId());
    ...
    /**
     * Get ZapusPerftest directory in which {@link ZapusPerftest} related files are saved.
     *
     * @param perfTest perfTest
     * @return directory
     */
    @Override
    public File getPerfTestDirectory(ZapusPerftest perfTest) {
     ...
     *
     * @param user user
     * @return deleted {@link ZapusPerftest} list
     */
    @Transactional
    public List<ZapusPerftest> deleteAll(ZapusUser user) {
        List<ZapusPerftest> perfTestList = getAll(user);
        perfTestRepository.flush();
        ZapusPerftestExample example = new ZapusPerftestExample();
        ZapusPerftestExample.Criteria criteria = example.createCriteria();
        criteria.andCreateUserIdEqualTo(user.getId());
        zapusPerftestMapperExt.deleteByExample(example);
     ...
     * @param perfTest perf test
     */
    public void cleanUpDistFolder(ZapusPerftest perfTest) {
     ...
     * @param perfTest perfTest
     */
    public void cleanUpRuntimeOnlyData(ZapusPerftest perfTest) {
        perfTest.setRunningSample("");
        perfTest.setAgentStat("");
        perfTest.setMonitorStat("");
        save(perfTest);
    }

    /**
     * Put the given {@link com.dianwoba.zapus.monitor.share.domain.SystemInfo} maps into the given
     * perftest entity.
     *
     ...
     */
    @SuppressWarnings({"unchecked", "rawtypes"})
    public Map<String, HashMap> getMonitorStat(ZapusPerftest perfTest) {
        return gson.fromJson(perfTest.getMonitorStat(), HashMap.class);
    }
    ...
    public int getMonitorGraphInterval(long testId, String targetIP, int imageWidth) {
        File monitorDataFile = new File(
            config.getHome().getPerfTestReportDirectory(String.valueOf(testId)),
     ...
     * @param dataInterval interval value to get data. Interval value "2" means, get one record for
     * every "2" records.
    ...
    public Map<String, String> getMonitorGraph(long testId, String targetIP, int dataInterval) {
        ...
        try {
            ...
            while (StringUtils.isNotBlank(line)) {
                if (skipCount < dataInterval) {
                    ...
                    if ("null".equals(datalist[4]) || "undefined".equals(datalist[4])) {
                        ...
                        sbUsedMem.append(Long.valueOf(datalist[4]) - Long.valueOf(datalist[3]))
                            .append(",");
                    }
            ...
            LOGGER
                .info("Error while getting monitor {} data file at {}", targetIP, monitorDataFile);
        } finally {
    ...
    private void completeCustomData(Map<String, String> returnMap, String key,
     ...
     * @return interval value.
     */
    public int getReportPluginGraphInterval(long testId, String plugin, String kind,
        int imageWidth) {
    ...
    /**
     * Get plugin monitor data and wrap the data as a string value like "[22,11,12,34,....]", which
     * can be used directly in JS as a vector.
     *
     * @param testId test id
     * @param plugin plugin name
     * @param kind kind
     * @param interval interval value to get data. Interval value "2" means, get one record for
     * every "2" records.
     * @return return the data in map
     */
    public Map<String, Object> getReportPluginGraph(long testId, String plugin, String kind,
        int interval) {
        ...
        try {
            ...
            String headerStringInJSONList = headerSB.deleteCharAt(headerSB.length() - 1).append("]")
                .toString();
     ...
     */
    public Pair<ArrayList<String>, ArrayList<String>> getReportData(long testId, String key,
        boolean onlyTotal, int interval) {
        Pair<ArrayList<String>, ArrayList<String>> resultPair = Pair.of(new ArrayList<String>(),
            new ArrayList<String>());
        List<File> reportDataFiles = onlyTotal ? Lists.newArrayList(getReportDataFile(testId, key))
            : getReportDataFiles(testId, key);
    ...
    public List<File> getReportDataFiles(long testId, String key) {
        ...
        Arrays.sort(files, new Comparator<File>() {
            ...
            public int compare(File o1, File o2) {
                return FilenameUtils.getBaseName(o1.getName())
                    .compareTo(FilenameUtils.getBaseName(o2.getName()));
            }
        });
    ...
    @Override
    public List<ZapusPerftest> getAll(Date start, Date end) {
        ZapusPerftestExample example = new ZapusPerftestExample();
        ZapusPerftestExample.Criteria criteria = example.createCriteria();
    ...
    @Override
    public List<ZapusPerftest> getAll(Date start, Date end, String region) {
        ZapusPerftestExample example = new ZapusPerftestExample();
        ZapusPerftestExample.Criteria criteria = example.createCriteria();
        criteria.andCreateDateBetween(start, end);
        criteria.andRegionEqualTo(region);

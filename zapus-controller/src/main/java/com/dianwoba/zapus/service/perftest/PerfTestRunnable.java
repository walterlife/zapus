...
import static com.dianwoba.zapus.model.Status.CANCELED;
import static com.dianwoba.zapus.model.Status.DISTRIBUTE_FILES;
import static com.dianwoba.zapus.model.Status.DISTRIBUTE_FILES_FINISHED;
import static com.dianwoba.zapus.model.Status.START_AGENTS;
import static com.dianwoba.zapus.model.Status.START_AGENTS_FINISHED;
import static com.dianwoba.zapus.model.Status.START_CONSOLE;
import static com.dianwoba.zapus.model.Status.START_TESTING;
import static com.dianwoba.zapus.model.Status.TESTING;
import static org.apache.commons.lang.ObjectUtils.defaultIfNull;
import static com.dianwoba.zapus.common.constant.ClusterConstants.PROP_CLUSTER_SAFE_DIST;

import com.dianwoba.zapus.extension.OnTestLifeCycleRunnable;
...
import com.dianwoba.zapus.model.perftest.NullSingleConsole;
import com.dianwoba.zapus.service.perftest.samplinglistener.AgentDieHardListener;
import com.dianwoba.zapus.service.perftest.samplinglistener.AgentLostDetectionListener;
import com.dianwoba.zapus.service.perftest.samplinglistener.MonitorCollectorPlugin;
import com.dianwoba.zapus.service.perftest.samplinglistener.PerfTestSamplingCollectorListener;
import com.dianwoba.zapus.service.perftest.samplinglistener.PluginRunListener;
...
public class PerfTestRunnable implements ControllerConstants {
    ...
    public void init() {
        ...
        scheduledTaskService
            .addFixedDelayedScheduledTask(startRunnable, PERFTEST_RUN_FREQUENCY_MILLISECONDS);
        ...
        scheduledTaskService
            .addFixedDelayedScheduledTask(finishRunnable, PERFTEST_RUN_FREQUENCY_MILLISECONDS);
    ...
    void doStart() {
        ...
        if (!canExecuteMore()) {
            ...
            LOG.debug("Currently running test is {}. No more tests can not run.",
    ...
    private boolean canExecuteMore() {
        return consoleManager.getConsoleInUse().size() < perfTestService
    ...
    protected boolean hasEnoughFreeAgents(PerfTest test) {
        ...
        if (test.getAgentCount() != null && test.getAgentCount() > size) {
            perfTestService.markProgress(test,
                "The test is tried to execute but there is not enough free agents."
                    + "\n- Current free agent count : " + size + "  / Requested : " + test
                    .getAgentCount() + "\n");
            return false;
        }
        return true;
    ...
    public void doTest(final PerfTest perfTest) {
        ...
        try {
            ...
            GrinderProperties grinderProperties = perfTestService
                .getGrinderProperties(perfTest, prepareDistribution);
            ...
            // In case of error, mark the occurs error on perftest.
            LOG.error("Error while executing test: {} - {} ", perfTest.getTestIdentifier(),
                e.getMessage());
    ...
    SingleConsole checkCancellation(SingleConsole singleConsole) {
        if (singleConsole.isCanceled()) {
            throw new SingleConsoleCancellationException(
                "Single Console " + singleConsole.getConsolePort()
                    + " is canceled");
        }
    ...
    SingleConsole startConsole(PerfTest perfTest) {
        perfTestService
            .markStatusAndProgress(perfTest, START_CONSOLE, "Console is being prepared.");
    ...
    void distributeFileOn(final PerfTest perfTest, SingleConsole singleConsole) {
        ...
        listener.add(new SingleConsole.FileDistributionListener() {
            ...
            public boolean start(File dir, boolean safe) {
                if (safe) {
                    perfTestService
                        .markProgress(perfTest, "Safe file distribution mode is enabled.");
                    return safe;
                }
                long sizeOfDirectory = FileUtils.sizeOfDirectory(dir);
                if (sizeOfDirectory > safeThreadHold) {
                    perfTestService
                        .markProgress(perfTest, "The total size of distributed files is over "
    ...
    protected long getSafeTransmissionThreshold() {
        return config.getControllerProperties()
            .getPropertyLong(PROP_CONTROLLER_SAFE_DIST_THRESHOLD);
    }
    ...
    void startAgentsOn(PerfTest perfTest, GrinderProperties grinderProperties,
        ...
        perfTestService
            .markStatusAndProgress(perfTest, START_AGENTS, getSafe(perfTest.getAgentCount())
                + " agents are starting.");
    ...
    void runTestOn(final PerfTest perfTest, GrinderProperties grinderProperties,
        ...
        for (OnTestLifeCycleRunnable run : pluginManager
            .getEnabledModulesByClass(OnTestLifeCycleRunnable.class)) {
            run.start(perfTest, perfTestService, config.getVersion());
        }

        // Run test
        perfTestService
            .markStatusAndProgress(perfTest, START_TESTING, "The test is ready to start.");
    ...
    protected void addSamplingListeners(final PerfTest perfTest,
        final SingleConsole singleConsole) {
        // Add SamplingLifeCycleListener
        singleConsole
            .addSamplingLifeCyleListener(new PerfTestSamplingCollectorListener(singleConsole,
                perfTest.getId(), perfTestService, scheduledTaskService));
        singleConsole
            .addSamplingLifeCyleListener(new AgentLostDetectionListener(singleConsole, perfTest,
        ...
        singleConsole
            .addSamplingLifeCyleListener(new PluginRunListener(testSamplingPlugins, singleConsole,
                perfTest, perfTestService));
        singleConsole.addSamplingLifeCyleListener(
            new AgentDieHardListener(singleConsole, perfTest, perfTestService,
    ...
    private boolean isTestFinishCandidate(PerfTest perfTest, SingleConsole singleConsoleInUse) {
        ...
        if (perfTest.isThresholdDuration()
            ...
            LOG.debug(
                ...
                new Object[]{perfTest.getTestIdentifier(),
     ...
     * @param singleConsoleInUse {@link SingleConsole} which is being used for the given {@link
     * PerfTest}
     */
    ...
    public void doTerminate(PerfTest perfTest, SingleConsole singleConsoleInUse) {
        ...
        try {
            perfTestService
                .markProgressAndStatusAndFinishTimeAndStatistics(perfTest, Status.STOP_BY_ERROR,
                    "Stopped by error");
        } catch (Exception e) {
    ...
    public void doNormalFinish(PerfTest perfTest, SingleConsole singleConsoleInUse) {
        ...
        try {
            ...
            if (perfTestService.hasTooManyError(perfTest)) {
                ...
                perfTestService
                    .markProgressAndStatusAndFinishTimeAndStatistics(perfTest, Status.FINISHED,

 ...
 */
package com.dianwoba.zapus.script.repository;
...
import com.dianwoba.zapus.common.exception.NGrinderRuntimeException;
...
import com.dianwoba.zapus.model.script.FileCategory;
import com.dianwoba.zapus.model.script.FileEntry;
import com.dianwoba.zapus.model.script.FileType;
import com.dianwoba.zapus.service.user.UserContext;
...
public class FileEntryRepository {
	 ...
	 */
	public File getUserRepoDirectory(ZapusUser user) {
	 ...
	 */
	public List<FileEntry> findAll(ZapusUser user, final String path, Long revision) {
	 ...
	 * @return found {@link FileEntry}s
	 */
	public List<FileEntry> findAll(ZapusUser user, final String path, Long revision, boolean recursive) {
	 ...
	 * @return found {@link FileEntry}s
	 */
	public List<FileEntry> findAll(final ZapusUser user) {
	 ...
	 */
	public FileEntry findOne(ZapusUser user, String path, SVNRevision revision) {
	...
	/**
	 * Save fileEntry on the location.
	 *
	 * @param user      the user
	 ...
	 */
	public void save(ZapusUser user, FileEntry fileEntry, String encoding) {
		SVNClientManager svnClientManager = null;
		...
		try {
			...
			for (String each : getPathFragment(fileEntry.getPath())) {
				...
				if (folderStepEntry != null && folderStepEntry.getKind() == SVNNodeKind.FILE) {
					throw processException("ZapusUser " + user.getUserId() + " tried to create folder "
							+ fullPath + ". It's file..");
				}
			}
	 ...
	 */
	public void delete(ZapusUser user, List<String> paths) {
		SVNClientManager svnClientManager = null;
		ISVNEditor editor = null;
		try {
			svnClientManager = getSVNClientManager();
	 ...
	 */
	public boolean hasOne(ZapusUser user, String path) {
		SVNClientManager svnClientManager = null;
		try {
			svnClientManager = getSVNClientManager();
			SVNURL userRepoUrl = SVNURL.fromFile(getUserRepoDirectory(user));
	 ...
	 */
	public void writeContentTo(ZapusUser user, String path, File toPathDir) {
		SVNClientManager svnClientManager = null;
